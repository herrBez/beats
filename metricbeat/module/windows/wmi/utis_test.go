...
func Test_Timeout(t *testing.T) {
	...
	startTime := time.Now()
	expectedError := fmt.Errorf("query '%s' exceeded the timeout of %s", query, timeout)
	_, err := ExecuteGuardedQueryInstances(mockSession, query, timeout)
	// Make sure the return time is less than the MockTimeout
	assert.Less(t, time.Since(startTime), MockTimeout, "The return time should be less or equal the sleep time")
	// Make sure the error returned is the expected one
	assert.Equal(t, err, expectedError, "Expected the returned error to match ErrMyError")
}
